...
static void ctr(struct xml_element *element)
{
	...
	struct xml_taxi_route_frame *taxi_route_frame = (struct xml_taxi_route_frame*)element;
	((struct xml_layout_frame*)taxi_route_frame)->mask |= XML_LAYOUT_FRAME_taxi_route_frame;
}
...
{
	return (struct ui_region*)ui_taxi_route_frame_new(interface, layout_frame->name, parent);
}

const struct xml_vtable xml_taxi_route_frame_vtable =
{
	.name = "TaxiRouteFrame",
	.ctr = ctr,
	.dtr = dtr,
