...
#define LUA_METHOD_MESSAGE_FRAME() LUA_METHOD(MessageFrame, message_frame)

#define UI_OBJECT (&UI_REGION->object)
#define UI_REGION (&UI_FRAME->region)
#define UI_FRAME (&message_frame->frame)
...
static bool ctr(struct ui_object *object, struct interface *interface, const char *name, struct ui_region *parent)
{
	if (!ui_frame_vtable.ctr(object, interface, name, parent))
		return false;
	struct ui_message_frame *message_frame = (struct ui_message_frame*)object;
	UI_OBJECT->mask |= UI_OBJECT_message_frame;
	return true;
}
...
{
	struct ui_message_frame *message_frame = (struct ui_message_frame*)object;
	return &message_frame->font_instance;
}

static int lua_AddMessage(lua_State *L)
{
	LUA_METHOD_MESSAGE_FRAME();
...
const struct ui_object_vtable ui_message_frame_vtable =
{
	UI_OBJECT_VTABLE("MessageFrame")
};
